/*we want to pull only the account name and the dates in which that account placed an order,
but none of the other columns, we can do this with the following query:*/
SELECT accounts.name, orders.occurred_at
FROM orders
JOIN accounts
ON orders.account_id = accounts.id;
/*This query only pulls two columns, not all the information in these two tables. Alternatively,
the below query pulls all the columns from both the accounts and orders table.*/
SELECT *
FROM orders
JOIN accounts
ON orders.account_id = accounts.id;
/*the first query you ran pull all the information from only the orders table*/
SELECT orders.*
FROM orders
JOIN accounts
ON orders.account_id = accounts.id;
...
JOIN accounts
ON orders.account_id = accounts.id
/*we wanted to join all three of these tables, we could use the same logic.
